...
Функция ``fetch`` вызванная от объекта ``PDOStatement`` вернёт следующую строку базы данных, он же массив с ключами, в
зависимости от
того, какой аргумент был передан в эту функцию. Ключами массива будут как раз наименования столбцов этой строки. Если
результатом выполнения SQL запроса будет множество строк, то
функция ``fetch`` будет содержать первую из них в виде массива с ключами или объекта. Если её вызвать ещё раз от того же
объекта PDOStatement, то мы получим следующую строку в базе
...
Обратите внимание, что приведённый ниже код будет выводить нам первую строку базы при каждом вызове. Это потому что мы
каждый раз заново выполняем запрос и просим из него первую строку.
