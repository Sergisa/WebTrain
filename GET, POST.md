...
используете PHP в качестве приложения сервера. Если вы запускали команду
...
* Порт базы данных (можно опустить если он **3306**)
* Имя базы данных
* Имя пользователя
* Пароль
...
Для создания подключения к базе в языке PHP используется класс PDO. Конструктор этого объекта требует в виде аргументов
строку для подключения, имя пользователя и пароль.

```injectablephp
$connection = new PDO(
    'mysql:host=192.148.74.89;dbname=students_test',
    'student',
    '20IT-PI(b/o)PIP-1'
)
```
...
Функция ```query``` лишь подготавливает запрос к базе данных, не извлекая из базы никаких данных. Текст SQL запроса
передаётся в функцию в качестве строки переданный в виде строки, в качестве аргумента. Данная функция возвращает объект
``PDOStatement``.

### FetchAll

Что бы исполнить запрос и получить записи из базы вызывается функция ``fetchAll``, которая получает все строки, которые
SQL запрос извлекает из базы. (Не все строки таблицы, а лишь те, которые требует SQL запрос).
Аргументом в данной функции передаётся константа ``PDO::FETCH_OBJ`` которая указывает, что результат от базы мы хотим
получить в виде **массива объектов**.  
Например: следующий код указывает что возвращаемый набор строк будет представлен в качестве массива объектов:
...
Если мы хотим получить ответ от базы в виде массива массивов, то в качестве аргумента указываем``PDO::FETCH_ASSOC``
...
Что бы было понятней вот пример такого массива:

```injectablephp
$students = [//массив строк
    [//массив столбцов одной строки
      'name'=>"...",
      'surname'=>"...",
      'group'=>"..."
    ],
...
Функция ``fetch`` вызванная от объекта ``PDOStatement`` вернёт первую строку запроса. Если результатом выполнения SQL
запроса будет множество строк, то функция ``fetch`` будет содержать первую из них в виде массива с ключами или объекта.
Если её вызвать ещё раз от того же объекта PDOStatement, то мы получим следующую строку в базе
...
Обратите внимание, что приведённый ниже код будет выводить нам первую строку базы при каждом `echo`. Это потому что мы
каждый раз заново формируем запрос, исполняем и просим из него первую строку.
...
Что бы такого не происходило можно написать так:

```injectablephp
$query = getPDO()->query('SELECT * FROM students')
echo $query->fetch(PDO::FETCH_ASSOC)['name'];
echo $query->fetch(PDO::FETCH_ASSOC)['name'];
echo $query->fetch(PDO::FETCH_ASSOC)['name'];
```

Теперь мы выполняем запрос один раз, а затем выбираем каждую последующую строчку из него не выполняя его заново.
...
указанием строки для него, а что просто символом кавычки. Следующий код не выполнится. PHP не поймёт где у функции
`query` заканчивается строка. После первой кавычки PHP сосчитает концом строки следующую такую кавычку.
...
* ``PDO::query()`` : Подготавливает выражение SQL без заполнителей
* ``PDO::prepare()`` : Подготавливает запрос к выполнению и возвращает связанный с этим запросом объект PDOStatement
    * ``PDOStatement::execute()`` : Запускает подготовленный запрос на выполнение (в качестве аргументов может быть
      массив,
      значениями которого нужно заполнить запрос)
    * ``PDOStatement::fetch()`` : Выполнение запроса и извлечение следующей строки из результирующего набора
    * ``PDOStatement::fetchAll()`` : Выполнение запроса и выбор всех строк из набора результатов
